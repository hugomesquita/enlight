    ...
    <form action="<?= base_url($tela . 'save') ?>" method="POST" enctype="multipart/form-data" name="frmRegistroModal" id="frmRegistroModal<?= date('dmY') ?>" data-form="frmRegistroModal">
        <div class="card-body">
            <div class="row" style="padding:10px">
                ...
                <div class="col-sm-10">
                    <label class="col-form-label">Termo*</label>
                    <input required name="Register[termo]" value="<?= $row->termo ?>" type="text" class="form-control form-control-sm" />
                </div>
                ...
                <div class="col-sm-12">
                    <label class="col-form-label">Exemplo*</label>
                    <select required name="Register[exemplo_id]" class="form-control form-control-sm">
                        <option></option>
                        <? foreach ($exemplos as $c) { ?>
                            <option <?= (($c->id == $row->exemplo_id) ? 'selected' : '') ?> value="<?= $c->id ?>"><?= $c->id.' - '.$c->exemplo_textual ?></option>
                        <? } ?>
                    </select>
                </div> 
