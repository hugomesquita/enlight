...
class Termos extends MS_Controller
{

    private $tela = 'painel/termos/';

    function __construct()
    {
        parent::__construct();
        $this->load->model('TermosModel', 'registro', FALSE);
        $this->load->model('ArtefatosModel', 'artefatos', FALSE);
        $this->load->model('CategoriasModel', 'categorias', FALSE);
        $this->load->model('ExemplosModel', 'exemplos', FALSE);
    ...
    function DataGridResult($get)
    {
        ...
        if ($get['description'] != '')
            $like[] = ['campo' => 'upper(termo)', 'valor' => strtoupper($get['description'])];
    ...
    function cadastro($id = null) 
    {
        ...
        $data = array(
            'titulo'   => $this->titulo,
            'tela'     => $this->tela, 
            'exemplos' => $this->exemplos->filtrar([])->result_object(),
